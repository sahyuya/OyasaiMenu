    ...
    override fun execute(source: CommandSourceStack, args: Array<out String>) {
        ...
        when {
            ...
            args.size >= 3 && args[1].toIntOrNull() != null -> {
                ...
                if (plugin.macroManager.getMacro(player.uniqueId, macroId) == null) { player.sendMessage("§cマクロ '§e$macroId§c' が見つかりません。"); return }
                if (value.equals("remove", ignoreCase = true)) {
                    val err = plugin.macroManager.removeCommandAtIndex(player.uniqueId, macroId, index)
                    if (err != null) player.sendMessage("§c$err") else player.sendMessage("§a'$macroId' の ${index+1} 番目を削除しました。")
                } else {
                    val err = plugin.macroManager.setCommandAtIndex(player.uniqueId, macroId, index, value)
                    if (err != null) player.sendMessage("§c$err") else player.sendMessage("§a'$macroId' の ${index+1} 番目を設定: §f$value")
                }
            }
            args.size == 2 -> { player.sendMessage("§7使い方: §f/macro §e<id> §b<番号> §f<コマンド>"); player.sendMessage("§7例: §f/macro home 1 //wand") }
            else -> {
